...
def negSamplingLossAndGradient(
    ...
    # loss = float(-np.log(sigmoid(vec[outsideWordIdx])) - np.sum(np.log(sigmoid(-vec[negSampleWordIndices]))))
    
    # gradCenterVec = - (1 - sigmoid(vec[outsideWordIdx])) *  u0+ np.sum( outsideVectors[negSampleWordIndices] * (1 - sigmoid(-vec[outsideWordIdx]) ))
    
    # mark = np.zeros(outsideVectors.shape)
    # gradOutsideVecs = np.zeros(outsideVectors.shape)

    # for i in negSampleWordIndices:
    #     mark[i, :] = mark[i, :] + 1
    # gradOutsideVecs[outsideWordIdx] = -centerWordVec * (1-sigmoid(vec[outsideWordIdx]))
    # gradOutsideVecs[negSampleWordIndices] += centerWordVec * (1-sigmoid(-vec))
    # gradOutsideVecs *= mark
    # # gradOutsideVecs = -np.outer( (1 - sigmoid(-vec)), centerWordVec ) * mark + ( 1- sigmoid(vec[outsideWordIdx])) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        outside_idx = word2Ind[outsideWord]
        local_loss, gradCenterVec, local_gradOutsideVecs = word2vecLossAndGradient(centerWordVectors[center_idx], outside_idx, outsideVectors, dataset)
